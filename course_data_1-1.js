...
========================================================================
核心作用：存放 Beginner 必修 1-1「基本世界觀與三方賽局」全量課程教材。
安全防線：內建萬能導通備援層，完美相容物件與純陣列路由，徹底絕殺 404 與未定義崩潰。
變更焊接：OQC 精確更名、Pin-to-Pin 實體絕殺、工廠端 IQC 尺寸與測站同步化重砲擴寫。
========================================================================
*/

// 👑 核心全量數據源大盤
const COMPATIBLE_PAGES_PAYLOAD = [
  // ─── 🗺️ PAGE 1 / 4: 大廳導讀 ───
  {
    ...
    tabs: [
      {
        ...
        keywords: [
          { 
            ...
            meaning: "客戶端發出的滾動式排產需求預測。供應商據此提前準備晶圓或長料，是供應鏈防禦的第一步。" 
          },
          { 
            name: "Buffer Stock", 
      ...
      },
      {
        ...
        expectation: "專案 BOM 表隨時維持雙料源熱備援状態，物料成本（BOM Cost）逐季剛性下壓。",
        keywords: [
          { 
            name: "AVL 名冊", 
      ...
      },
      {
        ...
        keywords: [
          { 
            ...
            meaning: "標準作業程序書。產線開線的鋼鐵規章，用圖文並茂的剛性步驟限制操作員的下壓行程與螺絲鎖付扭力。" 
          },
          { 
            name: "氣密測試治具", 
...
// ========================================================================
// 👑 萬能自適應導通熱備援映射（絕殺所有主殼變數不對位問題）
// ========================================================================
// 1. 如果主殼讀取的是 window.courseEngineData 物件
window.courseEngineData = {
  courseId: "1-1",
  pages: COMPATIBLE_PAGES_PAYLOAD
};

// 2. 如果主殼原本直接讀取 window.courseData 陣列
window.courseData = COMPATIBLE_PAGES_PAYLOAD;
