 ...
 * 定版代碼代號: 0528-V1.0
 * 單元代號: 1-1 (初心者寶典 - 系統硬體開發世界觀與 NPI 壓縮鏈條)
 * 權責範圍: 本檔案為純數據層，供通用外殼 beginner_codex.html 動態調用掛載
 */

codexCourseEngineData = [
    {
        courseId: "L1-A1", // 精準聯動與對齊天賦樹技能星盤的 Node ID，以利成績回寫
        menuTitle: "單元 1-1: 基礎通識世界觀模型",
        subPages: [
            {
                title: "1. 產品供應鏈定位模型",
                diagram: "【 視覺藍圖 b1-1_page1_blueprint : 客戶端 (設計端) ──> 零件廠商 (上游物料) ──> 製造工廠 (實體複製) 】",
                text: "在硬體專案管理的世界中，運行著「三方一鏈」的實務實體化鏈條。\n\n1. 客戶端 (Client / Brand R&D)：權責包括市場調查、外觀工業設計（ID）與結構設計（ME）。其核心職責是將凡間使用者的原始痛點與慾望，量化為初始的產品規格書（PRD），並輸出神聖的 2D/3D 工程圖紙（2D Drawing）。客戶追求的是極致的美學與領先的規格。\n\n2. 零件廠商 / 供應商 (Component Vendors)：控制特定核心元件晶圓材料的技術研發端（如：相機鏡頭模組、射頻感測器、特定防水膠材）。負責進貢樣品（Samples），並在大量生產時向代工廠輸送零件物料（Components）。其長料期（Lead Time）穩定度直接左右產線生死。\n\n3. 代工製造廠 (EMS Factory)：即我們（Foxconn / FIH）製造基地。接過客戶端的圖紙指令，吞下廠商的零件原料。在物理公差公差、操作工時與生產時程的邊界下，進行硬體實體化的大規模複製組裝。\n\n身為代工廠的 System TPM，核心任務是：當試產不良發生時，發起跨部門協調，透過數據分析找出不良的根本原因（Root Cause），並在不觸動客戶外觀設計的前提下，提出內部結構修正或規格優化的解決方案，最終主導可靠性驗證（Reliability Testing），確保產品安全落地。"
            },
            {
                title: "2. NPI 四大生命週期分期",
                diagram: "【 視覺藍圖 b1-1_page2_blueprint : 時程被極壓縮矩陣對比 Timeline Matrix 】",
                text: "新產品導入（NPI / New Product Introduction）是指一款產品從無到有的完整研發製造生命週期。標準的硬體開發包含四大階段，每個階段都設有嚴格的出關評審基準（Exit Criteria）：\n\n1. Proto (原型機驗證)：利用 3D 列印或手工打樣製作的初級樣機，用來快速確認產品外觀與結構概念。\n\n2. EVT (Engineering Verification Test / 工程驗證測試)：核心目標是實現主機板的「通電打通（Power-on）」。確認基本電路與硬體功能正常，除錯接口（Debug Interface）能順利抓到訊號。\n\n3. DVT (Design Verification Test / 設計驗證測試)：此時量產模具正式開模（Hard Tooling）。產品必須組裝完整，並接受最嚴苛的信賴性與可靠性拷問 ── 包括摔落測試（Drop Test）、高低溫環境衝擊、防水氣密測試（Leak Test）等。\n\n4. PVT (Production Verification Test / 生產驗證測試)：設計圖紙全面凍結（Freeze）。重心完全轉移到工廠，目的在於測試產線的每小時產能速率（UPH）、治具複製能力、操作工位線平衡（Line Balance），確保工廠具備大量複製的製程能力。\n\n⚠️ 產業真實壓縮現實：\n在高度成熟、高壓的行動裝置產業線中，標準流程已被市場競爭粉碎。哪怕是 Proto 階段，我們也被迫要一邊通電跑訊號，一邊開Hard Tooling鋼模進行機構驗證。EVT 與 DVT 階段發生大面積「平行重疊（Overlap）」，這是必須面對的修羅場。"
            },
            {
                title: "3. 生產線體與組裝工序流向",
                diagram: "【 視覺藍圖 b1-1_page3_blueprint : Neubrutalism 剛性方塊 Process Block-Flow Diagram 】",
                text: "硬體產品的架構不同，決定了工廠流水線不同的組裝與測試工序流向（Process Flow / Route）。System TPM 必須先看懂該產品別的生產線結構，才能在產線現場精準定位瓶頸工位。\n\n1. 通用硬體四大線體 (Generic Flow)\n- 主板站（PCBA Process）：在無塵車間進行鋼網錫膏印刷、高速貼片機元件拍打，最後過迴流焊熔爐（Reflow Oven），鑄造出產品「大腦」。\n- 組裝站（Assembly Line）：將主板、外殼、螢幕、排線與防護螺絲拼裝成一個實體機身。\n- 測試站（Test Station）：將機器插入自動化測試治具，強行檢驗其螢幕、喇叭、Wifi 訊號等功能。\n- 包裝站（Packaging）：套上防塵膜、塞入彩盒與配件，最後裝箱封膠出貨。\n\n2. 高階手機微縮精細線體 (Smartphone Flow)\n手機為了在極小空間塞入最多功能，線體被拆解為：SMT 主板製程 ─> SUB 次級預組線 ─> CG/BG 玻璃蓋板真空防水壓合 ─> Final Assembly 最終組裝線 ─> Final Test 最終功能與射頻天線測試 ─> Packing 外盒包裝出貨。"
            },
            {
                title: "4. 智慧無人機機電共生線體",
                diagram: "【 視覺藍圖 b1-1_page4_blueprint : 無人機動力分流與 IMU 校正站特殊高亮框線圖 】",
        ...
        // 👑 0528-V1.0 20題終極硬體情境大考題庫 ── 題題直擊 Foxconn/FIH 實戰血淚
        examVault: [
            ...
            { q: "工廠回報最終入庫良率高達 99%，但直通率（FPY）只有 30%，這代表什麼工程警訊？", o: ["產品品質極其良好，製程非常健康", "超過三分之二的機器被手工拆解重修過，帶有隱形物理內傷，極易發生開箱即損（DOA）", "可以直接開閘放水進入 MP 大量大量生產"], a: 1 },
            ...
            { q: "當專案進度正式跨入 PVT 階段時，代表以下何種硬體狀態已經發生？", o: ["產品還在進行最初步的市場概念調研", "研發設計檔案全面凍結（Freeze），重心全力轉移至工廠製程能力複製與拉速", "外殼噴砂顆粒度與材質尚未決定"], a: 1 },
