...
========================================================================
2026-05-28 V3.6: 👑 終極大修復：更正 .idx-floating-controls 類別呼叫，沒收黑怪獸橫條，解決中英文切換次像素抖動。
...
const globalUiTranslations = {
    tw: { 
        ...
        "auth-locked": "🔒 未讀取公會憑證", 
        "rank-default": "冒險者", 
        "logo-mark": "GUILD HQ / LEVEL 2026", 
        "main-title": "System TPM 公會總部", 
        "subtitle": "Daniel Shih | 2026 年度高效管理與團隊修煉套件" 
    },
    en: { 
        "nav-home": "Home", 
        "auth-locked": "🔒 No Credentials", 
        "rank-default": "Adventurer", 
        "logo-mark": "GUILD HQ / LEVEL 2026", 
        "main-title": "System TPM Guild", 
        "subtitle": "Daniel Shih | 2026 Tactical Management & Training Suite" 
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (pageName === 'index.html' || pageName === '') {
        ...
        if (!homeHeaderSection) {
            ...
            // 👑 修正：綁定正確的 idx-floating-controls 👑
            homeHeaderSection.innerHTML = `
                ...
                <div class="logo-mark" id="home-logo-mark-text" style="font-size: 0.9rem; font-weight: 900; letter-spacing: 5px; text-transform: uppercase; margin-bottom: 12px; color: var(--text-secondary); font-family: monospace;"></div>
                <h1 style="font-size: 3.2rem; margin: 0; font-weight: 900; letter-spacing: -1px;" id="home-main-title-text"></h1>
                <div class="subtitle" id="home-subtitle-text" style="font-size: 1.1rem; color: var(--text-secondary); font-weight: 700; margin-top: 15px;"></div>
            `;
    ...
    // 🚀 模式 B：子系統功能頁面
        ...
        if (pageName.includes('talent_tree')) { 
            sysTag = "SYS.ATLAS_TREE"; 
            sysTitleTw = "公會技能星盤天賦樹"; 
        } else if (pageName.includes('beginner_codex')) { 
            sysTag = "SYS.BEGINNER_CODEX"; 
            sysTitleTw = "初心者寶典大廳"; 
        } else if (pageName.includes('handbook')) { 
            sysTag = "SYS.HANDBOOK"; 
            sysTitleTw = "冒險者手冊大廳"; 
        } else if (pageName.includes('Library')) { 
            sysTag = "SYS.LEXICON_LIBRARY"; 
            sysTitleTw = "大圖書館術語圖鑑庫"; 
        }

        targetNavbar.innerHTML = `
            <div class="navbar-brand">
                <span style="font-family: monospace; font-weight: 900; color: var(--text-secondary); font-size: 0.8rem;">${sysTag} V3.6</span>
...
function syncGlobalLanguageState(lang) {
    ...
    if (homeBtn) {
        homeBtn.innerText = globalUiTranslations[lang]["nav-home"];
    }
    ...
    if (titleEl) {
        ...
        if (pageName.includes('talent_tree')) { 
            titleEl.innerText = lang === 'tw' ? "公會技能星盤天賦樹" : "Adventurer Talent Tree"; 
            document.title = titleEl.innerText; 
        } else if (pageName.includes('beginner_codex')) { 
            titleEl.innerText = lang === 'tw' ? "初心者寶典大廳" : "Beginner's Codex Hall"; 
            document.title = titleEl.innerText; 
        } else if (pageName.includes('handbook')) { 
            titleEl.innerText = lang === 'tw' ? "冒險者手冊大廳" : "Adventurer's Handbook Hall"; 
            document.title = titleEl.innerText; 
        } else if (pageName.includes('Library')) { 
            titleEl.innerText = lang === 'tw' ? "大圖書館術語圖鑑庫" : "Knowledge Lexicon Library"; 
            document.title = titleEl.innerText; 
        }
    }
    ...
    if (profileCard) {
        if (cachedName) {
            ...
            if (rankTag === "冒險者" || rankTag === "Adventurer") { 
                rankTag = (lang === 'tw') ? "冒險者" : "Adventurer"; 
            }
